...
function attachListenersToDocument(doc, currentFrameSelector = null) {
    ...
    const recordClick = (e) => {
        ...
        sendStep({ type: 'click', selector: getSelector(target), textContent: target.textContent ? target.textContent.trim() : '', timestamp: new Date().toISOString() }, frameSelector);
    };
...
function setupListenersInWindow(win) {
    attachListenersToDocument(win.document);
        ...
        try {
            ...
            iframes.forEach(iframe => {
                ...
                iframe.addEventListener('load', () => {
                    try {
                        attachListenersToDocument(iframe.contentDocument);
                    } catch (e) {
